...
contract EsAvtBatchSender {
    using SafeMath for uint256;

    address public admin;
    address public esAvt;

    constructor(address _esAvt) public {
        admin = msg.sender;
        esAvt = _esAvt;
    }

    modifier onlyAdmin() {
        require(msg.sender == admin, "EsAvtBatchSender: forbidden");
        _;
    }
    ...
    function send(
        ...
        for (uint256 i = 0; i < _accounts.length; i++) {
            IERC20(esAvt).transferFrom(msg.sender, _accounts[i], _amounts[i]);
