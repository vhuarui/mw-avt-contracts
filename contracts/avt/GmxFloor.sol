...
contract AvtFloor is ReentrancyGuard, TokenManager {
    ...
    modifier onlyHandler() {
        require(isHandler[msg.sender], "AvtFloor: forbidden");
        _;
    }
    ...
    function setBackedSupply(uint256 _backedSupply) public onlyAdmin {
        require(_backedSupply > backedSupply, "AvtFloor: invalid _backedSupply");
        backedSupply = _backedSupply;
    }

    function setMintMultiplier(uint256 _mintMultiplier) public onlyAdmin {
        require(_mintMultiplier > mintMultiplier, "AvtFloor: invalid _mintMultiplier");
        mintMultiplier = _mintMultiplier;
    }

    // mint refers to increasing the circulating supply
    // the AVT tokens to be transferred out must be pre-transferred into this contract
    function mint(uint256 _amount, uint256 _maxCost, address _receiver) public onlyHandler nonReentrant returns (uint256) {
        require(_amount > 0, "AvtFloor: invalid _amount");
        ...
        require(cost <= _maxCost, "AvtFloor: _maxCost exceeded");
        ...
        IERC20(avt).transfer(_receiver, _amount);
    ...
    function burn(uint256 _amount, uint256 _minOut, address _receiver) public onlyHandler nonReentrant returns (uint256) {
        ...
        require(amountOut >= _minOut, "AvtFloor: insufficient amountOut");

        backedSupply = backedSupply.sub(_amount);

        IMintable(avt).burn(msg.sender, _amount);
