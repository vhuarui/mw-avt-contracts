...
contract StakedAlpRewardDistributor2 is IRewardDistributor, ReentrancyGuard, Governable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    address public override rewardToken;
    uint256 public maxTokensPerInterval;
    ...
    event MaxTokensPerIntervalChange(uint256 amount);
    ...
    function tokensPerInterval() public view override returns (uint256) {
        ...
        if (supply < 100_000_000 * 10 ** 18) {
            return supply.mul(maxTokensPerInterval).div(100_000_000 * 10 ** 18);
        }
        return maxTokensPerInterval;
    }

    function setMaxTokensPerInterval(uint256 _amount) external onlyAdmin {
        require(lastDistributionTime != 0, "RewardDistributor: invalid lastDistributionTime");
        IRewardTracker(rewardTracker).updateRewards();

        maxTokensPerInterval = _amount;
        emit MaxTokensPerIntervalChange(_amount);
    }
