...
const deployFunction: DeployFunction = async function ({
  ...
  await sendTxn(
    secondaryPriceFeed.initialize(contractConfig.fastPriceFeed.minAuthorizations, signers, updaters),
    'secondaryPriceFeed.initialize',
  )
  await sendTxn(
    secondaryPriceFeed.setMaxTimeDeviation(contractConfig.fastPriceFeed.maxTimeDeviation),
  ...
  await sendTxn(
    secondaryPriceFeed.setPriceDataInterval(contractConfig.fastPriceFeed.priceDataInterval),
  ...
  for (const token of tokenArr) {
    ...
    await sendTxn(
      vault.setBufferAmount(token.address, token.bufferAmount),
...
deployFunction.dependencies = ['InitVault', 'FastPriceEvents', 'PositionRouter', 'VaultPriceFeed']
